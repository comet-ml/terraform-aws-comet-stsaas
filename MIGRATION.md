# Brownfield migration to v6.0.3 (DND-1573 / DND-1686)

For clusters still on a v1.20.x or v2.1.x module version. Greenfield clusters and
anything already on v5.x/v6.x do not need this — bump straight to `v6.0.3`.
...
`v6.0.3-migration.1` is `v6.0.3` plus:
...
Earlier migration tags are superseded: `v6.0.1-migration{,-3,-4}` were all cut from
`v6.0.0` and predate the Auto Mode and addon-conflict fixes that landed in `v6.0.3`.
Do not use them.
...
```hcl
module "comet" {
  source = "github.com/comet-ml/terraform-aws-comet-stsaas?ref=v6.0.3-migration.1"
...
```hcl
source = "github.com/comet-ml/terraform-aws-comet-stsaas?ref=v6.0.3"
```
