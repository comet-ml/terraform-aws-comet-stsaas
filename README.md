...
| <a name="input_rds_engine"></a> [rds\_engine](#input\_rds\_engine) | Engine type for RDS database. Also supplies the parameter group family prefix, so a value other than aurora-mysql needs rds\_parameter\_group\_family checked too. | `string` | `"aurora-mysql"` | no |
...
| <a name="output_mysql_proxy_in_use"></a> [mysql\_proxy\_in\_use](#output\_mysql\_proxy\_in\_use) | Whether mysql\_host resolves to the RDS Proxy (true) or the Aurora cluster writer endpoint (false). |
